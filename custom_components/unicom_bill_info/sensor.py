...
async def async_setup_entry(
    ...
    # 将 refresh_interval 转换为 timedelta 对象
    interval = timedelta(seconds=refresh_interval)
    async_track_time_interval(hass, update_callback, interval)
            ...
            async with self._session.post(self._url, json=payload) as response:
                ...
                if data["code"] == "0000":
                    if self._url == USAGE_URL:
                        if self._data_key == "x_canuse_value_data_2":
                            ...
                            self._state = float(re.sub(r'[^\d.]', '', value))
                        elif self._data_key in ["ADDUP_UPPER_0", "ADDUP_UPPER_2"]:
                            index = int(self._data_key.split("_")[-1])
                            value = data["data"][index]["ADDUP_UPPER"]
                            self._state = float(re.sub(r'[^\d.]', '', value))
                        elif self._data_key == "call_usage_percentage":
                            used_call_time_str = data["data"][0]["X_USED_VALUE"]
                            total_call_time_str = data["data"][0]["ADDUP_UPPER"]
                            used_call_time = float(re.sub(r'[^\d.]', '', used_call_time_str))
                            total_call_time = float(re.sub(r'[^\d.]', '', total_call_time_str))
                            if total_call_time > 0:
                                self._state = round((used_call_time / total_call_time) * 100, 2)
                            else:
                                self._state = 0
